...
type DDKGoodsSearchRequest struct {
	...
	RangeList        *[]DDKGoodsSearchRange `json:"range_list,omitempty"`         //范围列表，可选值：[{"range_id":0,"range_from":1,"range_to":1500},{"range_id":1,"range_from":1,"range_to":1500}]
	...
	MerchantTypeList *int                   `json:"merchant_type_list,omitempty"` //店铺类型数组
	Pid              *string                `json:"pid,omitempty"`                //推广位id
	BlockCats        *[]int64               `json:"block_cats,omitempty"`         //自定义屏蔽一级/二级/三级类目ID，自定义数量不超过20个;使用pdd.goods.cats.get接口获取cat_id
	BlockCatPackages *[]int                 `json:"block_cat_packages,omitempty"` //屏蔽商品类目包：1-拼多多小程序屏蔽类目;2-虚拟类目;3-医疗器械;4-处方药;5-非处方药
	GoodsImgType     *int                   `json:"goods_img_type,omitempty"`     //商品主图类型：1-场景图，2-白底图，默认为0
	GoodsSignList    *[]string              `json:"goods_sign_list,omitempty"`    //商品goodsSign列表
	UseCustomized    *bool                  `json:"use_customized,omitempty"`     //是否使用个性化推荐，默认true
}

type DDKGoodsSearchRange struct {
	RangeTo   *int64 `json:"range_to,omitempty"`   //如果左区间不限制，range_from传空就行，右区间不限制，range_to传空就行
	RangeFrom *int64 `json:"range_from,omitempty"` //如果左区间不限制，range_from传空就行，右区间不限制，range_to传空就行
	RangeID   *int   `json:"range_id,omitempty"`   //查询维度ID，枚举值如下：0-商品拼团价格区间，1-商品券后价价格区间，2-佣金比例区间，3-优惠券金额区间，4-加入多多进宝时间区间，5-销量区间，6-佣金金额区间，7-店铺描述评分区间，8-店铺物流评分区间，9-店铺服务评分区间
}

type DDKGoodsSearchResult struct {
	BaseResponse
	GoodsSearchResponse struct {
		GoodsList  []DDKGoodsSearchInfo `json:"goods_list"`  //商品列表
...
type DDKGoodsSearchInfo struct {
	...
	MallID                      int64    `json:"mall_id,omitempty"`                        //店铺id
